...
<table class="table table-sm mt-3 tabelas">
	...
	if($txtbuscar == ''){
		$res = $pdo->query("SELECT * from adicionais order by id desc LIMIT $limite, $itens_por_pagina");
	}else{
		$txtbuscar = '%'.@$_POST['txtbuscar'].'%';
		$res = $pdo->query("SELECT * from adicionais where nome LIKE '$txtbuscar' order by id desc");
	...
	$dados = $res->fetchAll(PDO::FETCH_ASSOC);
		...
		$res_todos = $pdo->query("SELECT * from adicionais");
		...
		if ($itens_por_pagina != 0) {
			...
			$num_paginas = 1;  // ou outro valor adequado, se aplicável ao seu caso
		}
	...
	for ($i=0; $i < count($dados); $i++) { 
			...
			$nome = $dados[$i]['nome'];
			$valor = $dados[$i]['valor'];
			$id_cat = $dados[$i]['id_cat'];

			$valor = number_format($valor, 2, ',', '.');
			
			//recuperar o nome da categoria
			$res_cat = $pdo->query("SELECT * from categorias where id = '$id_cat'");
