...
<table class="table table-sm mt-3 tabelas">
	...
	if($txtbuscar == ''){
		$res = $pdo->query("SELECT * from sabores order by nome asc LIMIT $limite, $itens_por_pagina");
	}else{
		$txtbuscar = '%'.@$_POST['txtbuscar'].'%';
		$res = $pdo->query("SELECT * from sabores where nome LIKE '$txtbuscar' order by nome asc");
	...
	$dados = $res->fetchAll(PDO::FETCH_ASSOC);
		...
		$res_todos = $pdo->query("SELECT * from sabores");
